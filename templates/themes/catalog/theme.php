<?php
	...
	class Catalog {
		public function build($settings, $board_name) {
			...
			$query = query(sprintf("SELECT *, `id` AS `thread_id`, `subject` AS `thread_subject`, (SELECT COUNT(*) FROM ``posts_%s`` WHERE `thread` = `thread_id`) AS `reply_count`, (SELECT COUNT(NULLIF(`file`, 0)) FROM ``posts_%s`` WHERE `thread` = `thread_id`) AS `image_count`, '%s' AS `board` FROM ``posts_%s`` WHERE `thread` IS NULL ORDER BY `bump` DESC", $board_name, $board_name, $board_name, $board_name)) or error(db_error());
